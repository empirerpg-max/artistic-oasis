  ...
  return (
    <main className="flex-1 mx-auto w-full max-w-2xl pb-32">
      <div
        ...
        style={{ background: `linear-gradient(180deg, oklch(0.32 0.16 145 / 0.7), oklch(0.12 0 0) 80%)` }}
      >
          ...
          {album.faixas.map((f, i) => {
            ...
            return (
              <li key={i} className={`group grid grid-cols-[2rem_1fr_auto_2.5rem] items-center gap-3 px-2 py-2 rounded-lg ${active ? "bg-primary/10" : "hover:bg-card"}`}>
                <button onClick={() => setPlayingIdx(active ? null : i)} className="size-8 grid place-items-center text-muted-foreground hover:text-foreground" aria-label={active ? "Pausar" : "Tocar"}>
                ...
                <a href={f.drive_url} target="_blank" rel="noreferrer" className="text-xs text-muted-foreground hover:text-primary justify-self-end" title="Abrir no Drive">
